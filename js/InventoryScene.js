...
export default class InventoryScene extends UIBaseScene {
  ...
  refresh() {
    ...
    for (let index = 0; index < this.maxColumns * this.rows; index++) {
      ...
      if(item){
        ...
        inventorySlot.desText = [];
        // inventorySlot.on("pointerover", pointer => {
        //   console.log(`pointerover:${index}`);
        //   console.log(item);
        //   inventorySlot.desText = this.add.text(inventorySlot.x,inventorySlot.y + this.tileSize/6,items[item.name].des,{
        //     font: "11px Courier",
        //     fill: "#111"
        //   }).setOrigin(0.5,0);
        //   this.hoverIndex = index;
  ...
  create() {
    this.scoreText = this.add.text(300,450,'Score: 0',{fontSize: '32px', fill:'#000'});
    this.alText = this.add.text(10,450,'I - expend the inventory\nC - Normal crafting\nV - Risky crafting',{fontSize: '15px', fill:'#000'});
    console.log("player score: "+this.player.score);
    this.scoreText.setText("Score: "+this.player.score);
    //SELECTION
    ...
    this.input.on("dragstart",()=>{
      ...
      this.inventorySlots[this.startIndex].desText =  this.add.text(this.inventorySlots[this.startIndex].x+32,this.inventorySlots[this.startIndex].y + this.tileSize/6,items[this.inventory.getItem(this.startIndex).name].des,{
            font: "11px Courier",
            fill: "#ffffff",
    ...
    this.input.on("dragend", () => {
      this.inventorySlots[this.startIndex].desText.destroy();
